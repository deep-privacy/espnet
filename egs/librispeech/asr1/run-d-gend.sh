...
datadir=/srv/storage/talc@talc-data.nancy/multispeech/corpus/speech_recognition
...
if [ ${stage} -le 4 ] && [ ${stop_stage} -ge 4 ]; then
    ...
    ${cuda_cmd} --gpu ${ngpu} ${expdir}/train.log \
        DAMPED_D_task='gender' DAMPED_N_DOMAIN=$damped_n_domain asr_train.py \
        --config ${train_config} \
...
if [ ${stage} -le 5 ] && [ ${stop_stage} -ge 5 ]; then
    ...
    (
        ...
        ${decode_cmd} JOB=1:${nj} ${expdir}/${decode_dir}/log/decode.JOB.log \
            DAMPED_D_task='gender' CUDA_VISIBLE_DEVICES=${#pids[@]} DAMPED_N_DOMAIN=$damped_n_domain asr_recog.py \
